...
st.sidebar.write("")
img_source=st.sidebar.radio("이미지 소스 선택해주세요", ("이미지 업로드","카메라로 촬영"))
if img_source=="이미지 업로드":
  img_file=st.sidebar.file_uploader("이미지를 선택해주세요",type=["png","jpg","jpeg"])
elif img_source == "카메라로 촬영":
  img_file=st.sidebar.camera_input("카메라로 촬영")
...
if img_file is not None:
  with st.spinner("측정 중..."):
    ...
    for result in results[:n_top]:
      ...
      st.write(f"{result[2]*100}% 의 확률로 {result[0]} 입니다.") 
